...
>
  ...
  ({bool isSet, LPCWSTR? get}) get identifier => LatePropertyAssigned<Pointer<Char>>(() => _identifier)
      ? (isSet: true, get: _identifier)
      : (isSet: false, get: null);

  late final Pointer<NativeType> _semaphore;

  ({bool isSet, Pointer<NativeType>? get}) get semaphore => LatePropertyAssigned<Pointer<NativeType>>(() => _semaphore)
      ? (isSet: true, get: _semaphore)
      : (isSet: false, get: null);

  WindowsSemaphore({required String name, required CTR counter, verbose = false})
  ...
  bool open() {
    ...
    if (!semaphore.isSet)
      _semaphore = Pointer.fromAddress(
        CreateSemaphoreW(
  ...
  bool openAttemptSucceeded() {
    ...
    if (verbose)
      print("Successfully [openAttemptSucceeded()] windows semaphore: ${name} at address: ${semaphore.get?.address}");
