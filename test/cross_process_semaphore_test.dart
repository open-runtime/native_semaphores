...
import 'package:test/test.dart' show equals, expect, group, isTrue, setUp, test;
...
void main() async {
  ...
  setUp(() async {
    final File primary_semaphore_executable = File(process_name);

    if (!primary_semaphore_executable.existsSync()) {
      final ProcessResult compiling_primary_semaphore_executable = Process.runSync('dart', [
        'compile',
        'exe',
  ...
  group('Testing Cross-Process Named Semaphore', () {
    test('Several Processes Coordinated Leveraging Named Locks', () async {
      final String name = '${safeIntId.getId()}_named_sem';
      ...
      final List<ProcessResult> processes = await Future.wait([
      ...
      final ProcessResult running_primary_semaphore_executable = processes.first;
      // Start with unlock delay of 4
      final ProcessResult running_secondary_semaphore_executable = processes.last;
      ...
      final RegExp lock_time = RegExp(r"Locking semaphore with name \d+_named_sem took: \[(\d+)\] seconds");

      final int? primary_lock_time = int.tryParse(
        lock_time.firstMatch(running_primary_semaphore_executable.stdout)?.group(1) ?? '-1',
      );
      final int? secondary_lock_time = int.tryParse(
        lock_time.firstMatch(running_secondary_semaphore_executable.stdout)?.group(1) ?? '-1',
      );
